...
export function stripModules(project: Project): void {
    ...
    console.log(JSON.stringify(configFileSet));
    ...
    for (const sourceFile of getTsSourceFiles(project)) {
        ...
        function collectReferencedNamespaces(node: Node, traversal: ForEachDescendantTraversalControl) {
            if (Node.isModuleDeclaration(node) && node.getParentIfKind(ts.SyntaxKind.SourceFile)) {
                ...
                node.setDeclarationKind(ModuleDeclarationKind.Namespace);
    ...
    newNamespaceFiles.forEach((reexports, filename) => {
        ...
        console.log("Associated config is ", JSON.stringify(associatedConfig));
        ...
        console.log("Simple config is ", JSON.stringify(simpleConfig));
        assert(simpleConfig);
