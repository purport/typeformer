...
function containsReferenceTo(name: string, debug: boolean) {
    ...
    function checkNode(n: Node): true | undefined {
        ...
        if (ts.isIdentifier(n) && ts.idText(n) === name &&
        ...
        if (ts.isBlock(n) || ts.isModuleBlock(n)) {
            ...
            if (n.statements.some(s => ts.isVariableStatement(s) && s.declarationList.declarations.some(d => bindingContainsName(d.name, name)))) {
                return;
            }
        }
